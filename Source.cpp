...
SlVector3 Tracer::shade(const HitRecord& hr) const {
    ...
    for (unsigned int i = 0; i < lights.size(); i++) {
        ...
        /*SlVector3 LitRay = light.p - hr.p;
        bool InsideSur = false;
        double n;
        double CosTheta = dot(LitRay, hr.n);
        HitRecord Hit;
        Hit.n = hr.n;
        if (CosTheta > 0.0) {
            n = hr.f.ior;
        ...
        double CosThetat = 1.0 - sqr(n) * (1.0 - sqr(CosTheta));
        if (CosThetat >= 0) {
            CosThetat = std::sqrt(CosThetat);
            SlVector3 RefractDir = - n * LitRay + (CosTheta * n - CosThetat) * Hit.n;
            Ray RefractRay(hr.p, RefractDir);
            normalize(RefractRay.d);
            color += hr.f.t * trace(RefractRay, this->shadowbias, MAX);
        }
        else {
            SlVector3 RefractDir = - LitRay + 2 * Hit.n * dot(Hit.n, LitRay);
            Ray ReflectRay(hr.p, RefractDir);
            normalize(ReflectRay.d);
            color += hr.f.t * trace(ReflectRay, this->shadowbias, MAX);
        }*/
    }

    // Step 4 Add code for computing reflection color here
    if (hr.raydepth < this->maxraydepth && hr.f.ks > 0) {
        SlVector3 DirRay = -hr.v + 2 * dot(hr.v, hr.n) * hr.n;
        Ray Refray(hr.p, DirRay);
        normalize(Refray.d);
        Refray.depth = hr.raydepth + 1;
        color += .5 * (hr.f.ks + hr.f.kd) * hr.f.ks * trace(Refray, this->shadowbias, MAX);
    }
...
int main(int argc, char* argv[]) {
    ...
    int maxraydepth = 5;
    bool color = false;
    Tracer tracer("D:\\James O'Brien\\tracefiles\\as2-InputFiles\\InputFiles\\mount.nnf");
    ...
    tracer.BVHTransfer();
    tracer.traceImage();
    tracer.writeImage("D:\\James O'Brien\\tracefiles\\as2-InputFiles\\InputFiles\\mount_BVH.ppm");
};
